                        ...
                        _ => {
                            if atom_name == "H1" || atom_name == "H2" || atom_name == "H3" {
                                ...
                                match AMBER_TYPES.get(&*atom_id) {
                                    Some(&t) => t,
                                    _ => panic!("CPYDOCK Error: Atom [{:?}] not supported", atom_id),
                                }
                            } else {
                                warn!("CPYDOCK Warning: Atom [{:?}] not supported, trying generic", atom_id);
